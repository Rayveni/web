...
class MongoDriver:
    ...
    def get_table(self,table,condition:dict={'_id': False},query:dict={},columns:list=None):
        if columns is not None:
            for field in columns:
                condition[field]=1
        return self.db[table].find(query,condition)
		
    def agg(self,table_name,group,agg_functions_list,sort_by=None):
    ...
    def merge(self,col_left,col_right,left_on,right_on,left_columns=None,right_columns=None,unwind=False,how='left',match_condition=None):
        col_l=self.db[self.db[table]]
